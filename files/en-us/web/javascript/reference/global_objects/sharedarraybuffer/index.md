...
`SharedArrayBuffer` objects must be constructed with the {{jsxref("Operators/new", "new")}} operator. Calling a `SharedArrayBuffer` as a function without using `new` will throw a {{jsxref("TypeError")}}.
